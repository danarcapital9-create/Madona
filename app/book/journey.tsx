...
import {ArrowUpLeft,ArrowDown,ArrowLeft,Pause,Play,MapPin,ArrowRight,Plus,Sparkle} from 'lucide-react';
...
export default function MadonnaJourney({onBook,motion,onMotion,bookingOpen,services}:{onBook:(category?:string,mode?:string)=>void;motion:boolean;onMotion:()=>void;bookingOpen:boolean;services:Service[]}){
 const root=useRef<HTMLDivElement>(null);useStudioMotion(root,motion,!bookingOpen);
 const [ritual,setRitual]=useState<string>('nails');const current=rituals.find(r=>r.id===ritual)!;
 return <div ref={root} className={'madonna-journey studio-edition '+(!motion?'motion-paused':'')+ (bookingOpen?' journey-covered':'')}><LivingSilk motion={motion} active={!bookingOpen}/><SignatureHero onBook={()=>onBook()} motion={motion} onMotion={onMotion} active={!bookingOpen}/>
 <section className="signature-bridge" aria-labelledby="signature-bridge-title"><span className="atelier-kicker">A SIGNATURE, ALL YOUR OWN</span><h2 id="signature-bridge-title"><span className="signature-bridge-line">تفاصيلكِ،</span> <em className="signature-bridge-line">تصنع الفرق.</em></h2><p>عناية تشبهكِ، من أول لمسة.</p><i className="studio-scroll-thread" aria-hidden="true"/></section>
 ...
 <ServiceStage selected={ritual} motion={motion} active={!bookingOpen} onSelect={setRitual} onBook={onBook}/>
 ...
 <div className="studio-marquee" aria-hidden="true" dir="ltr"><div className="studio-marquee-track">Be Bold. <Sparkle size={22}/> Be Beautiful. <Sparkle size={22}/> <em>Be Madonna.</em> <Sparkle size={22}/> Be Bold. <Sparkle size={22}/> Be Beautiful.</div></div>
