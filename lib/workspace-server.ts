...
import type {DeskAccess} from '@/lib/desk-access';
import {IdentityError,requireVerifiedCustomer} from '@/lib/customer-identity';
import {normalizePhone} from '@/lib/phone';
import {workspaceId} from '@/lib/admin-access';
...
function ownerOf(r:Request,audience:Audience){const id=r.headers.get('oai-authenticated-user-id');if(!id)throw new ApiError('سجّلي الدخول لفتح المساحة الخاصة.',401);const owner=workspaceId();if(!owner)throw new ApiError('المساحة قيد الإعداد.',503);return owner;}
...
async function state(owner:string,audience:Audience,receiptId?:string,access?:DeskAccess){
 const db=database();
 ...
 const sql=audience==='admin'?'SELECT * FROM bookings WHERE owner=?'+(access?.role==='specialist'?' AND staff=?':'')+' ORDER BY date DESC,start ASC LIMIT 1501':"SELECT date,staff,block_start,block_end,status FROM bookings WHERE owner=? AND date>=? AND date<=? AND status NOT IN ('cancelled','noshow') ORDER BY date,start";
 const stmt=db.prepare(sql);const rows=(await (audience==='admin'?(access?.role==='specialist'?stmt.bind(owner,access.staffId):stmt.bind(owner)):stmt.bind(owner,day(),day(60))).all<Record<string,unknown>>()).results;
 ...
 return {services,bookings:(audience==='admin'?rows.slice(0,1500):rows).map(decode),...(audience==='admin'?{historyLimited:rows.length>1500}:{}),today:day(),demo:true,...(access?{access}:{}),...(receipt?{receipt:decode(receipt),bookingId:receiptId}:{})};
}
function fail(e:unknown){if(e instanceof ApiError||e instanceof IdentityError||e instanceof DeskAccessError)return Response.json({error:e.message},{status:e.status});console.error('Madonna request failed',e);return Response.json({error:'تعذّر الاتصال بالمساحة الآن. بقيت بياناتك في النموذج؛ حاولي مجددًا.'},{status:503});}
export async function handleGet(r:Request,audience:Audience){try{const access=audience==='admin'?await deskAccess(r.headers):undefined;return Response.json(await state(ownerOf(r,audience),audience,undefined,access),{headers:{'Cache-Control':'private, no-store'}});}catch(e){return fail(e);}}
export async function handlePost(r:Request,audience:Audience){try{const access=audience==='admin'?await deskAccess(r.headers):undefined;const owner=ownerOf(r,audience);safeWrite(r);const input=await r.json() as {action:string;service:Service;id:string;status:string;customer:string;phone:string;date:string;start:number;staff:string;mode:string;address?:string;notes?:string;ids:string[];requestId:string};if(!input||typeof input!=='object')throw new ApiError('صيغة الطلب غير صالحة.');if(audience==='customer'&&!['initialize','book'].includes(input.action))throw new ApiError('الطلب غير مسموح.',403);if(access?.role!=='manager'&&access&&['samples','service'].includes(input.action)||access?.role==='specialist'&&input.action==='book')throw new ApiError('هذا الإجراء غير متاح لصلاحيتك.',403);const db=database();if(input.action==='initialize'){
 ...
 const initialized=await db.batch(stmts);return Response.json({...(await state(owner,audience,undefined,access)),initialized:initialized[0].meta.changes===1});
 }
 ...
 await db.batch(stmts);return Response.json(await state(owner,audience,undefined,access));
 }
 ...
 await db.prepare('UPDATE services SET price=?,duration=?,active=?,home=? WHERE owner=? AND id=?').bind(s.price,s.duration,s.active,s.home,owner,s.id).run();return Response.json(await state(owner,audience,undefined,access));
 }
 if(input.action==='status'){
 const b=await db.prepare('SELECT status,staff FROM bookings WHERE owner=? AND id=?').bind(owner,String(input.id)).first<{status:string;staff:string}>();if(!b||access?.role==='specialist'&&b.staff!==access.staffId)throw new ApiError('الحجز غير موجود.',404);if(input.status==='confirmed'&&access?.role==='specialist')throw new ApiError('تأكيد الطلبات من صلاحية الاستقبال.',403);if(!transitions[b.status]?.includes(input.status))throw new ApiError('لا يمكن تغيير حالة هذا الحجز بهذا الترتيب.',409);
 const res=await db.prepare('UPDATE bookings SET status=? WHERE id=? AND owner=? AND status=?').bind(input.status,input.id,owner,b.status).run();if(!res.meta.changes)throw new ApiError('تغيّر الحجز. حدّثي الصفحة.',409);return Response.json(await state(owner,audience,undefined,access));
 }
 ...
 const id=owner+':'+r.headers.get('oai-authenticated-user-id')+':'+requestId;const existing=await db.prepare('SELECT id FROM bookings WHERE owner=? AND id=?').bind(owner,id).first();if(existing)return Response.json({...(await state(owner,audience,id,access)),bookingId:id});
 ...
 return Response.json({...(await state(owner,audience,id,access)),bookingId:id},{status:201});
 }
